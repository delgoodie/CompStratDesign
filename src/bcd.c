...
void pathwiseprox_bcd3(double *rr, int *dd, int *M, double *lambda, int *p, double *sumvv, double *obj, float* penalty, int *maxiter, double *tol, int *verbose)
{
    ...
    double *rr0 = (double *)malloc(cp2 * sizeof(double)); // an original copy of rr
    for (i = 0; i < cp2; i++)
        rr0[i] = rr[i];
...
void allocate_groups(int *dd, int p, int *M, int ****elem, int ***nelem, int ***ngelem)
{
    ...
    *ngelem = (int **)malloc(p * sizeof(int *));
    ...
    for (j = 0; j < p; j++)
    {
        if (M[j] > 0)
        {
            ...
            (*nelem)[j] = (int *)malloc(M[j] * sizeof(int));
            (*ngelem)[j] = (int *)malloc(M[j] * sizeof(int));
            counter[j] = (int *)malloc(M[j] * sizeof(int));
            // initialize as zero
            for (b = 0; b < M[j]; b++)
            {
                (*nelem)[j][b] = 0;
    ...
    *elem = (int ***)malloc(p * sizeof(int **));
    for (j = 0; j < p; j++)
    {
        if (M[j] > 0)
        {
            // allocate *elem[j], which is an array of M[j] one-dimensional int arrays.
            (*elem)[j] = (int **)malloc(M[j] * sizeof(int *));
            for (b = 0; b < M[j]; b++)
            {
                (*elem)[j][b] = (int *)malloc((*nelem)[j][b] * sizeof(int));
            }
        }
    }
    ...
    {
        if (M[j] > 0)
            free(counter[j]); // since we had malloc(M[j] * sizeof(int));
    }
...
double objective(double ***v, double lam, int p, int *M, int ***elem, int **nelem, int **ngelem, double *rr0)
{
    ...
    int cp2 = p * (p - 1) / 2;
    double obj = 0, tt;
    double *sumvv = (double *)malloc(cp2 * sizeof(double));
    compute_sumvv(v, p, M, elem, nelem, ngelem, sumvv);
    for (i = 0; i < cp2; i++)
    {
        tt = rr0[i] - sumvv[i];
